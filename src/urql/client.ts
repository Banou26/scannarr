...
export const keyResolvers = {
  ...
  MediaPage: () => null,
  Media: (media) => (media as Media)._id, // ?? (media as Media).uri
  Episode: (episode) => (episode as Episode)._id, // ?? (episode as Episode).uri
  EpisodePage: () => null,
  PlaybackSourcePage: () => null,
  PlaybackSource: (playbackSource) => (playbackSource as PlaybackSource)._id, // ?? (playbackSource as PlaybackSource).uri
  Team: (team) => (team as Team)._id, // ?? (team as Team).uri
  MediaExternalLink: (mediaExternalLink) => (mediaExternalLink as MediaExternalLink)._id, // ?? (mediaExternalLink as MediaExternalLink).uri
  MediaTrailer: (mediaTrailer) => (mediaTrailer as MediaTrailer)._id, // ?? (mediaTrailer as MediaTrailer).uri
