...
export const mergeMap = {
  Media: {
    ...
    // this episodes has been autogenerated by copilot and somehow fixed the preview-modal episodes display, but havent really looked at its impl, might be wrong
    episodes: {
      compare: (previousValue, newValue) =>
        newValue.every(newNode => previousValue?.some(node => node._id === newNode._id)) &&
        previousValue?.every(node => newValue.some(newNode => newNode._id === node._id)),
